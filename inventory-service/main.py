...
async def connect():
	global connection
	print("[Inventory service] Connecting to RabbitMQ...")
	for _ in range(100):
		try:
			connection = await pk.connect_robust("amqp://guest:guest@rabbitmq/")
			print("[Inventory service] Connected to RabbitMQ.")
			return
		except Exception:
			print(f"[Inventory service] Waiting for RabbitMQ...")
			await asyncio.sleep(5)
	raise RuntimeError("[Inventory service] Failed to connect to RabbitMQ.")
